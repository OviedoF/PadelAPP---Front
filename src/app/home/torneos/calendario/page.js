...
export default function TorneosDivisiones() {
  ...
  const getUniqueCategories = () => {
    const categories = proximosTorneos.map(torneo => torneo.categoria)
              ...
              <div className="flex justify-center space-x-4 mb-4">
                {Array.from(new Set(divisionesData.map(d => d.division))).map((division) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-medium transition-colors ${selectedDivision === division
              ...
              <div className="flex justify-center space-x-4 mb-4">
                {['Masculino', 'Femenino'].map((genero) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center ${selectedGender === genero
                        ? 'bg-cyan-500 text-white'
                        : 'bg-gray-200 text-gray-700 hover:bg-cyan-100'
                      }`}
                  >
                          ...
                          <tr key={equipo.nombre} className="hover:bg-gray-50">
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center">
                                <span className={`flex items-center justify-center w-8 h-8 rounded-full ${index === 0 ? 'bg-yellow-100 text-yellow-800' :
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {proximosTorneos
                    .filter(torneo => torneo.division === selectedDivision && torneo.genero === selectedGender &&
                      (selectedCategory === 'Todas' || torneo.categoria === selectedCategory))
                    .map((torneo) => (
